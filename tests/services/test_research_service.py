...
def postgres_url() -> AsyncIterator[str]:
    with PostgresContainer("postgres:16-alpine") as p:
        yield p.get_connection_url().replace("postgresql+psycopg2://", "postgresql+asyncpg://")
...
async def test_legal_request_requires_jurisdiction_before_provider_or_persistence() -> None:
    ...
    service = ResearchService(
        None, provider, _settings(), quota_service=_NoQuota(), cache_service=_NoCache()
...
async def test_provider_failure_releases_reservation_and_is_structured() -> None:
    ...
    service = ResearchService(
        None, provider, _settings(), quota_service=quota, cache_service=_NoCache()
    )  # type: ignore[arg-type]
    with pytest.raises(ResearchServiceError) as exc_info:
        await service.execute(
            owner=ResearchOwnerContext(startup_id=uuid.uuid4(), user_id=uuid.uuid4()),
...
async def test_success_cache_force_refresh_and_extract_use_shared_result_contract() -> None:
    ...
    assert (
        success.accounting.credits_charged == 1
        and success.accounting.cost_usd is None
        and success.accounting.pricing_unknown
        and quota.reconciliations == [(1, True)]
    )
    ...
    refreshed = await service.execute(
        owner=owner, request=ResearchRequest(query="market", force_refresh=True)
    )
    assert (
        refreshed.accounting.provider_call_made is True
        and provider.search_calls == 2
        and quota.reserve_calls == 2
    )
    ...
    assert (
        "StageService" not in source
        and "DocumentService" not in source
        and "research_calls" not in source
    )
...
async def test_url_only_request_uses_canonical_url_for_cache_key_and_query_fingerprint() -> None:
    ...
    assert (
        outcome.accounting.query_fingerprint
...
def test_research_request_rejects_neither_query_nor_extraction_url() -> None:
    ...
    with pytest.raises(
        ValidationError, match="Provide a non-empty query or at least one extraction URL"
    ):
        ResearchRequest()
...
async def test_real_session_spy_records_only_research_tables(session_factory) -> None:
    async with session_factory() as seed:
        user = User(name="u", email=f"{uuid.uuid4()}@x.com")
        seed.add(user)
        await seed.flush()
        startup = Startup(user_id=user.id, name="s")
        seed.add(startup)
        await seed.commit()
    async with session_factory() as raw:
        spy = _SessionSpy(raw)
        service = ResearchService(spy, FakeProvider(_provider_response()), _settings())
        await service.execute(
            owner=ResearchOwnerContext(startup_id=startup.id, user_id=user.id),
            request=ResearchRequest(query="market"),
        )
...
class _SessionSpy:
    ...
    def add(self, instance, *args, **kwargs):
        self.models.append(type(instance).__name__)
        return self._session.add(instance, *args, **kwargs)
...
class _NoCache:
    def __init__(self, cached_result=None) -> None:
...
def _settings() -> Settings:
    return Settings(
        _env_file=None,
        JWT_SECRET="service-test-secret-with-at-least-thirty-two-bytes",
