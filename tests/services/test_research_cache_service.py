...
async def test_cached_hit_preserves_retrieval_and_write_ttl(session_factory) -> None:
    startup_id = await _startup(session_factory)
    written = datetime(2026, 8, 6, tzinfo=UTC)
    read = written + timedelta(seconds=10)
    async with session_factory() as session:
        service = ResearchCacheService(session, _settings())
        result = _result(written)
        await service.put(
        ...
        row = await session.scalar(select(ResearchCacheEntry))
        assert row.expires_at == written + timedelta(seconds=601)
        hit = await service.get(startup_id=startup_id, cache_key="key", served_at=read)
        assert (
            hit is not None
            and hit.retrieved_at == written
            and hit.served_at == read
            and hit.cache_hit
        )
...
async def test_expired_entry_is_miss_and_concurrent_upsert_is_atomic(session_factory) -> None:
    startup_id = await _startup(session_factory)
    then = datetime(2026, 8, 6, tzinfo=UTC)
    async with session_factory() as session:
        s = ResearchCacheService(session, _settings())
        await s.put(
            startup_id=startup_id,
            cache_key="key",
            operation="search",
        ...
        assert (
            await s.get(
                startup_id=startup_id, cache_key="key", served_at=then + timedelta(seconds=603)
            )
            is None
        )
    ...
    async def put(i):
        async with session_factory() as session:
            ...
            await ResearchCacheService(session, _settings()).put(
                startup_id=startup_id,
                cache_key="same",
                operation="search",
                category=ResearchCategory.GENERAL,
                result=_result(then + timedelta(seconds=i)),
                now=then,
            )
            ...
            len(
                (
                    await session.execute(
                        select(ResearchCacheEntry).where(ResearchCacheEntry.cache_key == "same")
                    )
                )
                .scalars()
                .all()
...
async def _startup(factory):
    async with factory() as s:
        u = User(name="u", email=f"{uuid.uuid4()}@x.com")
        s.add(u)
        await s.flush()
        x = Startup(user_id=u.id, name="x")
        s.add(x)
        await s.commit()
        return x.id
...
def _key_args() -> dict[str, object]:
    return {
        "startup_id": uuid.UUID("00000000-0000-0000-0000-000000000001"),
        "provider": "provider",
        "operation": "search",
        "query_or_url": "A query",
        "locale": "en-US",
        "topic": "general",
        "start_date": None,
        "end_date": None,
...
def _settings() -> Settings:
    return Settings(
        _env_file=None,
        JWT_SECRET="cache-test-secret-with-at-least-thirty-two-bytes",
        LLM_BASE_URL="http://localhost:20128/v1",
        LLM_MODEL="proxy-model",
        LLM_API_KEY="proxy-key",
        RESEARCH_CACHE_TTL_GENERAL_SECONDS=601,
        RESEARCH_CACHE_TTL_NEWS_SECONDS=602,
        RESEARCH_CACHE_TTL_PRICING_SECONDS=603,
        RESEARCH_CACHE_TTL_LEGAL_SECONDS=604,
    )
